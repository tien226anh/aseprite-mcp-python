# Aseprite MCP Project Guide for Codex CLI
...
- Wrap mutations in `app.transaction()`, always save after mutations
- Account for cel position offset

### Indexing
- Frame indices: **1-based**, Pixel coordinates: **0-based**

### String Escaping & Path Traversal
- Normalize Windows backslashes, escape with `_lua_escape()`, reject `..`

### Color Validation — Two Systems, Don't Mix
- New: `validate_hex_color(color)` → `(r, g, b)` or `None`
- Legacy: `utils.parse_hex_color(color)` → raises `ValueError`

### Layer Targeting
- By **name string**, not index. Case-sensitive.
...
### Naming: `snake_case.aseprite`, `{character}_{action}.aseprite`, PascalCase layers, snake_case tags

### Directory: `generated_assets/{project_name}/` with hero/, monsters/, environment/, effects/, cutscene/

### Canvas Sizes
| Asset Type | Typical Size | Frames |
|-----------|-------------|--------|
| Items | 16×16 to 24×24 | 1-4 |
| Characters | 32×32 to 48×48 | 4-8 |
