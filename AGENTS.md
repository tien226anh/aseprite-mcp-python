...
- **Server**: `aseprite_mcp/server.py` -- FastMCP server with resources, prompts, and legacy tools (sprite_create, sprite_export, sprite_info, sprite_list_layers, sprite_list_tags, spritesheet_export, script_execute, ws_connect, draw_pixels, fill_rect)
...
- **Helper utilities**: `aseprite_mcp/tools/_helpers.py` -- shared `get_cli()`, `check_file()`, `validate_hex_color()`, `_lua_escape()`
...
- **Lua escape helper**: `_lua_escape()` in `_helpers.py` escapes double quotes and backslashes in strings embedded in Lua scripts to prevent injection.
